...
#ifdef PREV_CONTROL
#define OPTIONS_AT_COMPILE                                                     \
  (ONIG_OPTION_IGNORECASE | ONIG_OPTION_EXTEND | ONIG_OPTION_MULTILINE |       \
   ONIG_OPTION_SINGLELINE | ONIG_OPTION_FIND_LONGEST |                         \
   ONIG_OPTION_FIND_NOT_EMPTY | ONIG_OPTION_NEGATE_SINGLELINE |                \
   ONIG_OPTION_DONT_CAPTURE_GROUP | ONIG_OPTION_CAPTURE_GROUP |                \
   ONIG_OPTION_WORD_IS_ASCII | ONIG_OPTION_DIGIT_IS_ASCII |                    \
...
#define OPTIONS_AT_COMPILE                                                     \
  (ONIG_OPTION_IGNORECASE | ONIG_OPTION_EXTEND | ONIG_OPTION_MULTILINE |       \
   ...
   ONIG_OPTION_SPACE_IS_ASCII | ONIG_OPTION_POSIX_IS_ASCII |                   \
   ONIG_OPTION_TEXT_SEGMENT_EXTENDED_GRAPHEME_CLUSTER |                        \
   ONIG_OPTION_TEXT_SEGMENT_WORD | ONIG_OPTION_IGNORECASE_IS_ASCII)
#endif

#define OPTIONS_AT_RUNTIME                                                     \
  (ONIG_OPTION_NOTBOL | ONIG_OPTION_NOTEOL |                                   \
   ONIG_OPTION_CHECK_VALIDITY_OF_STRING | ONIG_OPTION_NOT_BEGIN_STRING |       \
   ONIG_OPTION_NOT_END_STRING | ONIG_OPTION_NOT_BEGIN_POSITION |               \
...
static void to_binary(unsigned int v, char s[/* 33 */]) {
  unsigned int mask;
  int i;
...
#ifdef DUMP_INPUT
static void dump_input(unsigned char *data, size_t len) {
  static FILE *DumpFp;
...
static void dump_file(char *path, unsigned char *data, size_t len) {
  FILE *fp;
...
static void dump_data(FILE *fp, unsigned char *data, int len) {
  int i;
...
static int progress_callout_func(OnigCalloutArgs *args, void *user_data) {
  return ONIG_CALLOUT_SUCCESS;
}

static int each_match_callback_func(const UChar *str, const UChar *end,
                                    const UChar *match_start,
...
static int search(regex_t *reg, unsigned char *str, unsigned char *end,
                  OnigOptionType options, int backward, int sl) {
  int r;
  ...
  if (r >= 0) {
    ...
    if (r == ONIGERR_STACK_BUG || r == ONIGERR_UNDEFINED_BYTECODE ||
...
static int exec(OnigEncoding enc, OnigOptionType options,
                OnigSyntaxType *syntax, char *apattern, char *apattern_end,
                char *adata_pattern, char *adata_pattern_end, char *astr,
                UChar *end, int backward, int sl) {
  int r;
  regex_t *reg;
  OnigErrorInfo einfo;
  ...
  r = onig_new(&reg, pattern, pattern_end, (options & OPTIONS_AT_COMPILE), enc,
               syntax, &einfo);
  if (r != ONIG_NORMAL) {
    char s[ONIG_MAX_ERROR_MESSAGE_LEN];
...
static size_t fix_size(size_t x, OnigEncoding enc, int sl, int backward) {
  if (x > MAX_REM_SIZE)
    x = MAX_REM_SIZE;

  if (sl > 0) {
    if (sl >= 256) { // 256: exists heavy element
      if (x > MAX_SLOW_REM_SIZE2)
        x = MAX_SLOW_REM_SIZE2;
    } else {
      if (x > MAX_SLOW_REM_SIZE)
        x = MAX_SLOW_REM_SIZE;
    }
  }
...
static int alloc_exec(OnigEncoding enc, OnigOptionType options,
                      OnigSyntaxType *syntax, int backward, int pattern_size,
                      size_t rem_size, unsigned char *data) {
  extern int onig_detect_can_be_slow_pattern(
      const UChar *pattern, const UChar *pattern_end, OnigOptionType option,
  ...
  sl = onig_detect_can_be_slow_pattern(pattern, pattern_end, options, enc,
                                       syntax);
#ifdef STANDALONE
  fprintf(stdout, "sl: %d\n", sl);
#endif
  ...
  if (
    ...
    data_pattern =
        (unsigned char *)malloc(data_pattern_size != 0 ? data_pattern_size : 1);
  ...
  r = exec(enc, options, syntax, (char *)pattern, (char *)pattern_end,
           (char *)data_pattern, (char *)data_pattern_end, (char *)str,
           str_null_end, backward, sl);
...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
  ...
  static OnigEncoding encodings[] = {
      ...
      ONIG_ENCODING_UTF8,        ONIG_ENCODING_UTF8,
      ONIG_ENCODING_UTF8,        ONIG_ENCODING_UTF8,
      ONIG_ENCODING_ASCII,       ONIG_ENCODING_EUC_JP,
      ...
      ONIG_ENCODING_UTF32_BE,    ONIG_ENCODING_UTF32_LE,
      ONIG_ENCODING_ISO_8859_1,  ONIG_ENCODING_ISO_8859_2,
  ...
  static OnigSyntaxType *syntaxes[] = {
      ...
      ONIG_SYNTAX_GNU_REGEX,      ONIG_SYNTAX_JAVA,     ONIG_SYNTAX_PERL_NG,
      ONIG_SYNTAX_PYTHON,         ONIG_SYNTAX_ONIGURUMA};

#ifdef STANDALONE
  static char *syntax_names[] = {"Posix Extended", "Emacs",    "Grep",
                                 "GNU Regex",      "Java",     "Perl+NG",
                                 "Python",         "Oniguruma"};
#endif
...
#ifdef STANDALONE
  {
    ...
    fprintf(stdout,
            ...
            ONIGENC_NAME(enc), syntax_names[syntax_choice % num_syntaxes],
  ...
  r = alloc_exec(enc, options, syntax, backward, pattern_size, rem_size, data);
  if (r == -2)
    exit(-2);

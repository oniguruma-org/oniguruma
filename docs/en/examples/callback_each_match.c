...
static int each_match_callback(const UChar *str, const UChar *end,
                               const UChar *match_start, OnigRegion *region,
                               void *user_data) {
#if 1
  fprintf(stdout, "each_match_callback:\n");
  fprintf(stdout, "  match at:  %ld - %d: %p\n", match_start - str,
          region->end[0], user_data);
...
static int search(UChar *pattern, UChar *str, OnigOptionType options,
                  OnigOptionType runtime_options) {
  int r;
...
static int match(UChar *pattern, UChar *str, UChar *at, OnigOptionType options,
                 OnigOptionType runtime_options) {
  int r;
  unsigned char *start, *range, *end;
  regex_t *reg;
...
extern int main(int argc, char *argv[]) {
  ...
  search(pattern, str, ONIG_OPTION_FIND_LONGEST,
         ONIG_OPTION_CALLBACK_EACH_MATCH);

  fprintf(stdout, "<match>\n");
  match(pattern, str, str + 5, ONIG_OPTION_NONE,
        ONIG_OPTION_CALLBACK_EACH_MATCH);

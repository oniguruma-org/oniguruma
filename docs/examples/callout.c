...
static int callout_body(OnigCalloutArgs *args, void *user_data) {
  int r;
  int i;
  int n;
  int begin, end;
  ...
  fprintf(stdout, "%s %s: contents: \"%s\", start: \"%s\", current: \"%s\"\n",
          contents != 0 ? "CONTENTS" : "NAME",
          in == ONIG_CALLOUT_IN_PROGRESS ? "PROGRESS" : "RETRACTION", contents,
          start, current);
...
static int retraction_callout_func(OnigCalloutArgs *args, void *user_data) {
  return callout_body(args, user_data);
}

static int foo(OnigCalloutArgs *args, void *user_data) {
  return callout_body(args, user_data);
}

static int bar(OnigCalloutArgs *args, void *user_data) {
  int r;
  int i;
  int n;
  OnigType type;
...
static int test(OnigEncoding enc, OnigMatchParam *mp, char *in_pattern,
                char *in_str) {
  int r;
...
extern int main(int argc, char *argv[]) {
  ...
  name = (UChar *)"foo";
  id = onig_set_callout_of_name(enc, ONIG_CALLOUT_TYPE_SINGLE, name,
  ...
  opt_defaults[0].s.end =
      opt_defaults[0].s.start + strlen((char *)opt_defaults[0].s.start);
  opt_defaults[1].c = 0x4422;

  id = onig_set_callout_of_name(
      enc, ONIG_CALLOUT_TYPE_SINGLE, name, name + strlen((char *)name),
      ONIG_CALLOUT_IN_PROGRESS, bar, 0, 3, arg_types, 2, opt_defaults);
  if (id < 0) {
    fprintf(stderr, "ERROR: fail to set callout of name: %s\n", name);
    // return -1;
  }
  ...
  if (r != ONIG_NORMAL) {
    fprintf(stderr,
            "ERROR: fail onig_set_callout_user_data_of_match_param(): %d\n", r);
  }
  ...
  test(enc, mp, "(?:(*MAX[A]{7})a|(*MAX[B]{5})b)*(*CMP{A,>=,4})",
       "abababcabababaa");

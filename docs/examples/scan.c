...
static int exec(OnigEncoding enc, OnigOptionType options,
                OnigOptionType runtime_options, char *apattern, char *astr) {
  int r;
  unsigned char *end;
  regex_t *reg;
  OnigErrorInfo einfo;
  ...
  r = onig_new(&reg, pattern, pattern_end, options, enc, ONIG_SYNTAX_DEFAULT,
               &einfo);
  if (r != ONIG_NORMAL) {
    char s[ONIG_MAX_ERROR_MESSAGE_LEN];
...
extern int main(int argc, char *argv[]) {
  exec(ONIG_ENCODING_UTF8, ONIG_OPTION_NONE, ONIG_OPTION_NONE, "\\Ga+\\s*",
       "a aa aaa baaa");
  fprintf(stdout, "\n");
  ...
  exec(ONIG_ENCODING_UTF8, ONIG_OPTION_NONE, ONIG_OPTION_NONE, "(?!\\G)a+\\s*",
       "a aa aaa baaa");
  fprintf(stdout, "\n");
  ...
  exec(ONIG_ENCODING_UTF8, ONIG_OPTION_NONE, ONIG_OPTION_NONE, "a+\\s*",
       "a aa aaa baaa");

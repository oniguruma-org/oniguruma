...
static int echo(OnigCalloutArgs *args, void *user_data) {
  int r;
  OnigCalloutIn in;
  OnigType type;
  OnigValue val;
...
static int test(OnigEncoding enc, char *in_pattern, char *in_str) {
  int r;
...
extern int main(int argc, char *argv[]) {
  ...
  id = onig_set_callout_of_name(
      ...
      ONIG_CALLOUT_IN_BOTH, echo, 0, 2, arg_types, 2, opt_defaults);
  if (id < 0) {
